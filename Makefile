...
build-ric: ## Build FlexRIC docker image
	...
	@docker image build -t muriloavlis/flexric:${FLEXRIC_DOCKER_VERSION} --build-arg FLEXRIC_VERSION=${FLEXRIC_VERSION} -f docker/Dockerfile.flexric.ubuntu .

build-emu-gnb-cu: ## Build FlexRIC-CU docker image
	@echo "Building FlexRIC-gNB-CU..."
	@docker image build -t muriloavlis/flexric-gnb-cu:${FLEXRIC_DOCKER_VERSION} --build-arg FLEXRIC_VERSION=${FLEXRIC_VERSION} -f docker/Dockerfile.emu-gnb-cu.ubuntu .

build-emu-gnb-du: ## Build FlexRIC-DU docker image
	@echo "Building gNB-DU..."
	@docker image build -t muriloavlis/flexric-gnb-du:${FLEXRIC_DOCKER_VERSION} --build-arg FLEXRIC_VERSION=${FLEXRIC_VERSION} -f docker/Dockerfile.emu-gnb-du.ubuntu .
...
run-emu-gnb-cu: ## Run FlexRIC-gNB-CU on Docker
	...
	@docker container run -d -it --name flexric-gnb-cu --privileged --restart always --env NEAR_RIC_IP=${NEAR_RIC_IP} muriloavlis/flexric-gnb-cu:${FLEXRIC_DOCKER_VERSION}

run-emu-gnb-du: ## Run FlexRIC-gNB-DU on Docker
	@echo "Running FlexRIC-gNB-DU on Docker..."
	@docker container run -d -it --name flexric-gnb-du --privileged --restart always --env NEAR_RIC_IP=${NEAR_RIC_IP} muriloavlis/flexric-gnb-du:${FLEXRIC_DOCKER_VERSION}
...
docker-push-all: ## Push images to Docker Hub
	...
	@docker image push muriloavlis/flexric:${FLEXRIC_DOCKER_VERSION}
	@docker image push muriloavlis/flexric-gnb-cu:${FLEXRIC_DOCKER_VERSION}
	@docker image push muriloavlis/flexric-gnb-du:${FLEXRIC_DOCKER_VERSION}
